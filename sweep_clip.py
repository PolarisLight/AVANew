...
def main():
    ...
    val_loader = DataLoader(dataset=val_dataset, batch_size=wandb.config.batch_size, shuffle=True,
                            num_workers=wandb.config.num_workers)
    model = AesClip(dropout=wandb.config.dropout_p).to(device)
    criterion = emd_loss()
    ...
    for epoch in range(wandb.config.epochs):
        ...
        with torch.no_grad():
            ...
            if acc > best_acc:
                best_acc = acc
                torch.save(model.state_dict(), os.path.join("saved_models", f"clip_model_{epoch}.pth"))
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project='Clip-sweep')
